...
const PARTY_COLORS: Record<number, { bg: string; text: string; border: string }> = {
...
export default function AdminReorderModal({
  ...
  useEffect(() => {
    if (isOpen && members) {
      // Only reorder pending & active members, or all members sorted by queue_order
  ...
  const itemInfo = ITEM_CONFIG[itemName] || { label: itemName, color: 'from-blue-500 to-indigo-600' }

  // Initial order map for checking changes
  const initialOrderMap = new Map(
    members.map((m, idx) => [m.id, m.queue_order || idx + 1])
  )

  const changedCount = items.reduce((acc, m, idx) => {
  ...
  const handleSortByPartyGeneral = () => {
    const sorted = [...items].sort((a, b) => {
      const pA = a.profiles?.party_id ?? 9999
      const pB = b.profiles?.party_id ?? 9999
      if (pA !== pB) return pA - pB

      const slotA = a.profiles?.slot_index ?? 9999
      const slotB = b.profiles?.slot_index ?? 9999
  ...
  const handleSortByGuildLeague = () => {
    const sorted = [...items].sort((a, b) => {
      const pA = a.profiles?.party_id_guild_league ?? 9999
      const pB = b.profiles?.party_id_guild_league ?? 9999
      if (pA !== pB) return pA - pB

      const slotA = a.profiles?.slot_index_guild_league ?? 9999
      const slotB = b.profiles?.slot_index_guild_league ?? 9999
      if (slotA !== slotB) return slotA - slotB
  ...
  const handleSortByEmperium = () => {
    const sorted = [...items].sort((a, b) => {
      const pA = a.profiles?.party_id_emperium_overrun ?? 9999
      const pB = b.profiles?.party_id_emperium_overrun ?? 9999
      if (pA !== pB) return pA - pB

      const slotA = a.profiles?.slot_index_emperium_overrun ?? 9999
      const slotB = b.profiles?.slot_index_emperium_overrun ?? 9999
      if (slotA !== slotB) return slotA - slotB
  ...
  const filteredIndices = useMemo(() => {
    ...
    items.forEach((m, idx) => {
      ...
      const party = String(m.profiles?.party_id || '')
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-3xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-5 space-y-1.5 min-h-[250px]">
          {items.map((member, index) => {
            ...
            const partyColor = partyId && PARTY_COLORS[partyId] ? PARTY_COLORS[partyId] : null
            const isMatch = filteredIndices ? filteredIndices.has(index) : true
            const isDragging = draggedIndex === index
            const isOver = dragOverIndex === index && draggedIndex !== index
            const target = member.base_quota + (member.transferred_in_quota || 0) - (member.transferred_out_quota || 0)
