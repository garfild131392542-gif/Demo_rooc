  ...
  return (
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 p-4 dark:bg-gray-950/20 transition-colors duration-300">
      ...
      <div className="w-full max-w-2xl bg-white p-8 md:p-12 rounded-3xl border border-slate-100 shadow-xl dark:bg-gray-900 dark:border-gray-800 transition-all duration-300">
        ...
        {/* Progress Section */}
        <div className="mb-10 border-b border-slate-100 pb-8 dark:border-gray-800">
        ...
        {currentStep === 1 && (
          <div className="flex flex-col gap-6 animate-in fade-in duration-300 relative">
            {/* Step Header */}
            <div className="mb-2">
              <h1 className="text-2xl font-extrabold text-gray-950 dark:text-white tracking-tight">ข้อมูลพื้นฐานของกิลด์</h1>
              <p className="text-slate-600 dark:text-slate-400 mt-1">กรุณาระบุชื่อและตั้งค่าลิงก์ URL สำหรับหน้าต่างกิลด์ของคุณ</p>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex items-center gap-1.5 text-xs mt-1.5 min-h-[18px]">
                {guildUrlStatus.checking && (
                  <span className="text-slate-500 flex items-center gap-1">🔄 กำลังตรวจสอบลิงก์...</span>
                )}
                {!guildUrlStatus.checking && guildUrlStatus.available === true && (
                  <span className="text-emerald-600 dark:text-emerald-400 font-bold flex items-center gap-1">✅ ลิงก์นี้สามารถใช้งานได้</span>
                )}
                {!guildUrlStatus.checking && guildUrlStatus.available === false && (
                  <span className="text-red-500 dark:text-red-400 font-bold flex items-center gap-1">❌ ลิงก์นี้ถูกผู้อื่นใช้งานไปแล้ว</span>
                )}
              </div>
            </div>
        ...
        {currentStep === 2 && (
          <div className="flex flex-col gap-6 animate-in fade-in duration-300">
            {/* Step Header */}
            <div className="mb-2">
              <h1 className="text-2xl font-extrabold text-gray-950 dark:text-white tracking-tight">ข้อมูลการติดต่อ</h1>
              <p className="text-slate-600 dark:text-slate-400 mt-1">ระบุอีเมลสำหรับจัดการระบบ และลิงก์โซเชียลมีเดีย (ถ้ามี)</p>
            </div>
            ...
            <div className="bg-emerald-50/60 dark:bg-emerald-950/20 p-6 rounded-2xl border border-emerald-100 dark:border-emerald-900/40 shadow-inner">
              ...
              <div className="flex gap-2 mt-2.5 items-start text-emerald-800 dark:text-emerald-300">
                ...
                <p className="text-xs font-medium leading-relaxed">
                  จำเป็นสำหรับการจัดการระบบ, ข้อมูลการต่ออายุแพลตฟอร์ม หรือกู้คืนระบบกรณีฉุกเฉิน (ข้อมูลนี้จะไม่ถูกเปิดเผยต่อสาธารณะ)
                </p>
              </div>
            </div>
            ...
            <FormInput
              label="ลิงก์ Facebook Page / Group (ตัวเลือกเพิ่มเติม)"
              name="facebookLink"
              type="url"
        ...
        {currentStep === 3 && (
          <div className="flex flex-col gap-8 items-center py-6 animate-in zoom-in-95 duration-300 relative">
            {!inviteLink ? (
              <>
                ...
                <div className="text-center space-y-2">
                  ...
                  <p className="text-base text-slate-600 dark:text-slate-400 max-w-md mx-auto leading-relaxed">
                    ข้อมูลกิลด์ของคุณพร้อมสำหรับการจัดตั้งระบบแล้ว กดปุ่มด้านล่างเพื่อเริ่มดำเนินการ
                  </p>
                </div>

                <div className="flex w-full gap-4 pt-4 max-w-md">
                  <button
                    onClick={handleBack}
                    disabled={isLoading}
                    className="flex-1 bg-slate-100 hover:bg-slate-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-slate-700 dark:text-white font-bold py-3.5 px-6 rounded-xl transition-all disabled:opacity-50 cursor-pointer text-base"
                  >
                    ย้อนกลับ
                  </button>
                  <button
                    ...
                    disabled={isLoading}
                    className="flex-[2] bg-blue-600 hover:bg-blue-700 disabled:bg-slate-400 text-white font-bold py-3.5 px-6 rounded-xl transition-all shadow-md disabled:cursor-not-allowed cursor-pointer text-base"
                  >
